...
def get_max_boxes(bounding_boxes):
    """ Get the biggest box """
    bounding_boxes = bounding_boxes[bounding_boxes[:, 4] > 0.8]
    area = np.ones(len(bounding_boxes))
    for i in range(0, len(bounding_boxes)):
        box = bounding_boxes[i]
        area[i] = (box[2] - box[0]) * (box[3] - box[1])
    return bounding_boxes[np.argsort(area)[-1]]
