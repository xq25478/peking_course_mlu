...
class Detector:
    def __init__(self, min_face_size=20.0, thresholds=[0.6, 0.7, 0.8], nms_thresholds=[0.7, 0.7, 0.7]):
        self.device = torch.device('mlu' if torch.mlu.is_available() else 'cpu')
    ...
    def detect_faces(self, image):
        ...
        for s in scales:  # run P-Net on different scales
            boxes = run_first_stage(image, self.pnet, scale=s, threshold=self.thresholds[0],device=self.device)
